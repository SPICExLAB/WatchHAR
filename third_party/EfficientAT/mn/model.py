...
class MN(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        if kwargs.get('feature_extraction', False):
            self.classifier = nn.Sequential(
                nn.AdaptiveAvgPool2d(1),
                nn.Flatten(start_dim=1),
                nn.Linear(lastconv_output_channels, last_channel),
...
def get_model(num_classes: int = 527, pretrained_name: str = None, width_mult: float = 1.0,
              ...
              input_dim_t: int = 1000, se_dims: str = 'c', se_agg: str = "max", se_r: int = 4, feature_extraction : bool = False):
    """
    ...
    m = mobilenet_v3(pretrained_name=pretrained_name, num_classes=num_classes,
                     ...
                     input_dims=input_dims, se_conf=se_conf, feature_extraction=feature_extraction
                     )
    # print(m)
    return m
