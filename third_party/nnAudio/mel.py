...
class MelSpectrogram(nn.Module):
    ...
    def forward(self, x):
        ...
        melspec = melspec.transpose(1, 2)  # Shape: (num_audio, time_steps, 64)
        melspec = torch.squeeze(melspec, 0)  # Resulting shape: (time_steps, 64)
        return melspec  # Return power spectrogram, not dB (matches audioIMU reference)

    def power_to_db(self, melspec):
        amin = 1e-10
        ref_value = 1.0
        log_spec = 10.0 * torch.log10(torch.clamp(melspec, min=amin, max=np.inf))
